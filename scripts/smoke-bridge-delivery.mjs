...
const clientId = `bridge-text-smoke-${Date.now()}`;
const text = "bridge text probe ignore: reply with BRIDGE_TEXT_OK only";
...
async function postCommand() {
  ...
  assertManifestContext(data.command, "bridge text create");
  return data.command;
}
...
async function pollCommand(id) {
  ...
  while ((Date.now() - startedAt) < 180000) {
    ...
    if (command) {
      assertManifestContext(command, "bridge text poll");
      ...
      if (status === "answered") {
        ...
        const matched = replies.find((reply) => /BRIDGE_TEXT_OK/i.test(String(reply?.text || "")));

        if (!matched) {
          throw new Error("Bridge text smoke answered, but BRIDGE_TEXT_OK reply was not found.");
        }
      ...
      if (status === "failed") {
        throw new Error(String(command.errorMessage || "Bridge text smoke failed."));
      }
    }
  ...
  throw new Error("Bridge text smoke timed out waiting for a reply.");
}

async function main() {
  console.log(`Submitting bridge text smoke command to ${BASE_URL}`);
  const command = await postCommand();
  console.log(`commandId=${command.id}`);
  const answered = await pollCommand(command.id);
  console.log(`Bridge text smoke OK: command ${answered.id} answered via stage=${answered.progressStage || "unknown"} dispatchMode=${answered.dispatchMode || "unknown"}`);
}
